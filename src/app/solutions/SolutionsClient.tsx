...
import {
  Boxes, GitMerge, Plug, Building2, Scale, Landmark, ShieldCheck, FileText,
  Search, PenTool, Rocket, Wrench, MoveRight, Check,
} from "lucide-react";
...
const process = [
  { Icon: Search, h: "Scope", b: "We look at your actual documents and data, and agree on what a working solution needs to do." },
  { Icon: PenTool, h: "Design", b: "We design the schema and the extraction rules, and confirm the approach before we build." },
  { Icon: Wrench, h: "Build", b: "Our team builds and tests against your real documents, not a generic sample set." },
  { Icon: Rocket, h: "Ship & support", b: "We deploy, connect it to your systems, and stay on to support it as your needs change." },
];
    ...
    <div className="overflow-x-clip" style={{ background: UI.bg, color: UI.ink }}>
      ...
      <main>
          ...
          <div className="relative max-w-[1280px] mx-auto px-5 sm:px-8 pt-36 sm:pt-44 pb-16 sm:pb-20">
            <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-y-10 lg:gap-x-16 items-center">
              <Reveal>
                ...
                <motion.p variants={fadeUp} custom={1} className="mt-6 font-sans-g leading-relaxed max-w-[50ch]" style={{ fontSize: "1.1rem", color: UI.sub }}>
                ...
                <motion.a variants={fadeUp} custom={2} href="/contact" className="inline-flex items-center gap-2 mt-8 font-sans-g text-[15px] font-semibold px-6 py-3.5 rounded-lg text-white" style={{ background: UI.blue }}>
              ...
              <Reveal key={tr.id}>
                <motion.article id={tr.id} variants={fadeUp} className="scroll-mt-28 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                  <div className={i % 2 === 1 ? "lg:order-2" : ""}>
                    ...
                    <div className="font-sans-g text-[12.5px] font-semibold uppercase tracking-[0.14em]" style={{ color: UI.blue2 }}>{tr.t}</div>
                    <h2 className="mt-3 font-display font-bold tracking-[-0.02em] leading-[1.08]" style={{ fontSize: "clamp(1.5rem, 2.2vw, 2.1rem)", color: UI.ink }}>{tr.h}</h2>
                    <p className="mt-4 font-sans-g leading-relaxed max-w-[48ch]" style={{ fontSize: "1rem", color: UI.sub }}>{tr.d}</p>
                  ...
                  <div className={i % 2 === 1 ? "lg:order-1" : ""}>
                    <div className="rounded-2xl p-8 sm:p-10" style={{ background: UI.surface, border: `1px solid ${UI.line}`, boxShadow: CARD }}>
                      <div className="grid place-items-center aspect-[4/3] rounded-xl" style={{ background: UI.soft }}>
                        <tr.Icon className="w-16 h-16" strokeWidth={1.1} style={{ color: UI.blue }} />
                      </div>
                    </div>
                  </div>
                </motion.article>
          ...
          <div className="max-w-[1280px] mx-auto px-5 sm:px-8">
            <Reveal className="max-w-2xl mb-14">
              ...
              <motion.p variants={fadeUp} custom={1} className="mt-5 font-sans-g leading-relaxed" style={{ fontSize: "1.05rem", color: UI.sub }}>
                Four steps, from a first conversation to a product your team runs day to day.
              </motion.p>
            </Reveal>
            <Reveal>
              <motion.div variants={fadeUp} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {process.map((s, i) => (
                  <div key={s.h} className="relative rounded-2xl p-7" style={{ background: UI.surface, border: `1px solid ${UI.line}`, boxShadow: CARD }}>
                    <div className="flex items-center justify-between mb-6">
                      <span className="grid place-items-center w-11 h-11 rounded-xl" style={{ background: UI.soft, color: UI.blue }}><s.Icon className="w-5 h-5" strokeWidth={1.6} /></span>
                      <span className="font-display text-[2rem] font-bold leading-none" style={{ color: UI.line2 }}>{i + 1}</span>
                    </div>
                    <h3 className="font-display text-[1.1rem] font-bold tracking-tight" style={{ color: UI.ink }}>{s.h}</h3>
                    <p className="mt-2 font-sans-g text-[0.88rem] leading-relaxed" style={{ color: UI.sub }}>{s.b}</p>
                  </div>
                ))}
              </motion.div>
            </Reveal>
        ...
        <CtaBand
          ...
          image="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=1400&q=70"
          imageAlt="An enterprise engineering team planning at a whiteboard"
        />
      </main>
      <SiteFooter />
    </div>
